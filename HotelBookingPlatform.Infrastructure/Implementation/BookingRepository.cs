namespace HotelBookingPlatform.Infrastructure.Implementation
{
    public class BookingRepository : GenericRepository<Booking>, IBookingRepository
    {
        ...
        public async Task UpdateBookingStatusAsync(int bookingId, BookingStatus newStatus)
        {
            ...
            //IMPORTANT: Never delete cancelled bookings.
            // We keep the record for audit (reason/who/when) and for reliable reports.
        ...
        public async Task<IEnumerable<Booking>> GetExpiredBookingsAsync(DateTime now)
        {
            return await _appDbContext.Bookings
                .Include(b => b.Rooms)
                .Where(b => b.CheckOutDateUtc <= now && b.Status != BookingStatus.Completed && b.Status != BookingStatus.Cancelled)
                .AsSplitQuery()
                .ToListAsync();
        }
        ...
        public async Task<List<Booking>> GetExpiredBookingsWithRoomsAsync(DateTime currentUtcTime)
        {
            return await _appDbContext.Bookings
                .Include(b => b.Rooms)
                .Where(b => (b.Status == BookingStatus.Pending || b.Status == BookingStatus.Confirmed) && b.CheckOutDateUtc <= currentUtcTime)
                .ToListAsync();
        }
